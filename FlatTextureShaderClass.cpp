...
FlatTextureShaderClass::FlatTextureShaderClass(ID3D11Device* device, Camera* cam)
{
	m_pVertexShader = 0;
	m_pPixelShader = 0;
	m_pVertexLayout = 0;
...
FlatTextureShaderClass::~FlatTextureShaderClass(void)
{
...
void FlatTextureShaderClass::initialize(ID3D11Device* device)
{
	// Compile the vertex shader
	HRESULT hr;
    ID3DBlob* pVSBlob = nullptr;
    hr = CompileShaderFromFile(L"FlatTextureShaders_VS.hlsl", "main", "vs_4_0", &pVSBlob);
    if(FAILED(hr))
    ...
    hr = CompileShaderFromFile(L"FlatTextureShaders_PS.hlsl", "main", "ps_4_0", &pPSBlob);
    if(FAILED(hr))
        MessageBox(nullptr, L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
	...
	bd.ByteWidth = sizeof(FlatTextureConstBuffer);
...
void FlatTextureShaderClass::setParameters(ID3D11DeviceContext* deviceContext, GameObject* p_object)
{
...
void FlatTextureShaderClass::render(ID3D11DeviceContext* deviceContext)
{
...
HRESULT FlatTextureShaderClass::CompileShaderFromFile(WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
    HRESULT hr = S_OK;
